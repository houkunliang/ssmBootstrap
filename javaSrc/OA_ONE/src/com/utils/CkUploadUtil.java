...
@WebServlet("/ckUplodUtil")
public class CkUploadUtil extends HttpServlet {
...
//        String savePath = this.getServletContext().getRealPath("/") + configPath;
        if(request.getParameter("upPath")!=null&&!request.getParameter("upPath").equals("")){
        	...
        	savePath = this.getServletContext().getRealPath("/") + configPath;
        }
        // 临时文件目录   
...
//        writer.write("{\"success\":\""+str.substring(0, str.length()-1)+"\"}");
        ...
        out.println("window.parent.CKEDITOR.tools.callFunction(" + callback + ",'" + request.getContextPath()+str.substring(0, str.length()-1) + "',''" + ")");
        out.println("</script>");
