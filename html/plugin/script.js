var App = function () {
	...
	/*-----------------------------------------------------------------------------------*/
	/*	Sidebar & Main Content size match侧边栏和主内容大小匹配
	/*-----------------------------------------------------------------------------------*/
	...
	/*-----------------------------------------------------------------------------------*/
	/*	Collapse Sidebar Programatically 侧边栏控制
	/*-----------------------------------------------------------------------------------*/
	...
	/*-----------------------------------------------------------------------------------*/
	/*	Responsive Sidebar Collapse 响应式变换
	/*-----------------------------------------------------------------------------------*/
...
//			jQuery('#main-content').addClass("margin-left-0");
			...
			var menu = $('.sidebar');
			if (menu.parent('.slimScrollDiv').size() === 1) { // destroy existing instance before resizing
				menu.slimScroll({//在调整大小之前销毁已有的实例
					destroy: true
				});
				menu.removeAttr('style');
				$('#sidebar').removeAttr('style');
...
//			console.log(width);
	...
	setInterval(windowSize,45000);//循环检测
	...
	/*-----------------------------------------------------------------------------------*/
	/*	Handle Fixed Sidebar on Mobile devices移动设备上的固定边栏
	/*-----------------------------------------------------------------------------------*/
	...
	var handleProgress = function () {
		$(document).ready(function(){
			jQuery.fn.anim_progressbar = function (aOptions) {
				// def values  def值
				var iCms = 1000; //1秒
				var iMms = 60 * iCms;//一分钟
				var iHms = 3600 * iCms;//一小时
				var iDms = 24 * 3600 * iCms;//一天

				// def options def选项
				var aDefOpts = {
					start: new Date(), // now 当前时间
					finish: new Date().setTime(new Date().getTime() + 60 * iCms), // now + 60 sec  当前时间戳转换为当前时间+1分钟,结束时间
					interval: 100
				}
				var aOpts = jQuery.extend(aDefOpts, aOptions);
				var vPb = this;

				// each progress bar 每一个进度条
				return this.each(
					function() {
						var iDuration = aOpts.finish - aOpts.start;

						// calling original progressbar 调用原始progressbar
						$(vPb).children('.pbar').progressbar();

						// looping process 循环过程
						var vInterval = setInterval(
							function(){
								var iLeftMs = aOpts.finish - new Date(); // left time in MS 离开时间
								var iElapsedMs = new Date() - aOpts.start, // elapsed time in MS 运行时间
									iDays = parseInt(iLeftMs / iDms), // elapsed days
									iHours = parseInt((iLeftMs - (iDays * iDms)) / iHms), // elapsed hours
									iMin = parseInt((iLeftMs - (iDays * iDms) - (iHours * iHms)) / iMms), // elapsed minutes
									iSec = parseInt((iLeftMs - (iDays * iDms) - (iMin * iMms) - (iHours * iHms)) / iCms), // elapsed seconds
									iPerc = (iElapsedMs > 0) ? iElapsedMs / iDuration * 100 : 0; // percentages 百分率

								// 显示当前位置和进步
								$(vPb).children('.percent').html('<b>'+iPerc.toFixed(1)+'%</b>');//toFixed(1):把百分率四舍五入为一位小数的数字
...
//			var curr_time = moment().format('YYYY-MM-DD HH:mm:ss');
			...
			var headImg=urlObj.getTokenObj().USER_HEADIMG;
			
			var msg = '';
                msg +='<li class="animated fadeInLeft media">';
				msg +=  '<a class="pull-right" href="#">';
				msg += headImg=='undefined'|| headImg==''||headImg==null?'<img class="media-object" style="border-radius:99px;width:60px;" alt="Generic placeholder image" src="img/avatars/avatar3.jpg">':'<img class="media-object" style="border-radius:99px;width:60px;" alt="Generic placeholder image" src="'+getImgUrl(urlObj.url+headImg)+'">';
//				msg +=	'<img class="media-object" style="border-radius:99px;width:60px;" alt="Generic placeholder image" src="img/chat/headshot2.jpg">';
				msg +=  '</a>';
				msg +=  '<div class="media-body chat-pop">';
				msg +=	'<h4 class="media-heading">我<span class="pull-right"><i class="fa fa-clock-o"></i> <abbr class="timeagos" title="'+curr_time+'" >'+curr_time+'</abbr> </span></h4></h4>';
				msg +=	text;
				msg +=  '</div>';
				msg +='</li>';
...
//		chart_revenue();
		...
		var chart1 = window.chart = $('#pir_yl_1').data('easyPieChart');
		//Pie 2
		$('#pir_yl_2').easyPieChart({
			easing: 'easeOutBounce',
			onStep: function(from, to, percent) {
				$(this.el).find('.percent').text(Math.round(percent)+"%");
			},
		...
		var chart2 = window.chart = $('#pir_yl_2').data('easyPieChart');
		//Pie 3
		$('#pir_yl_3').easyPieChart({
			easing: 'easeOutBounce',
			onStep: function(from, to, percent) {
				$(this.el).find('.percent').text(Math.round(percent)+"%");
			},
		...
		var chart3 = window.chart = $('#pir_yl_3').data('easyPieChart');
	...
	return {
        ...
        init: function () {
			...
			checkLayout();	//Function to check if mini menu/fixed header is activated 函数检查是否激活了迷你菜单/固定头
			handleSidebar(); //Function to display the sidebar显示侧边栏的功能
			handleSidebarCollapse(); //Function to hide or show sidebar 功能隐藏或显示侧边栏
			handleSidebarAndContentHeight();  //Function to hide sidebar and main content height 用于隐藏侧栏和主要内容高度的函数
			responsiveSidebar();		//Function to handle sidebar responsively 处理侧边栏响应的功能
			handleTeamView(); //Function to toggle team view 切换团队视图的功能
			handleHomePageTooltips(); //Function to handle tooltips 处理工具提示的功能
			handleBoxTools(); //Function to handle box tools 处理盒工具的功能
...
(function () {
    ...
    this.ThemeArr = (function () {
        function ThemeArr() {}
        ThemeArr.colors = ["#FFFFFF","#5E87B0","#D9534F","#A8BC7B","#70AFC4","#F0AD4E","#FCD76A","#6B787F","#D4E5DE","#A696CE","#DB5E8C","#F38630"];
        return ThemeArr;
    })();
})(window.jQuery);